...
int pulseDurations[8];

// Scenario A: All valid values (no zero)
int dataValues_A[8] = {10, 30, 50, 70, 90, 100, 20, 40};

// Scenario B: One zero value (motor 1 disabled)
int dataValues_B[8] = {10, 0, 25, 50, 75, 100, 90, 60};

bool previousButtonStates[8] = {true};

// === EXPERIMENT CONFIGURATION ===
int currentScenario = 0; // 0 = Scenario A, 1 = Scenario B
int currentGroup = 0;
int currentTrial = 0;
...
void setupScenario(int scenario) {
  for (int i = 0; i < 8; i++) {
    ...
    if (value == 0) {
      ...
      pulseDurations[i] = map(value, 1, 100, 100, 800);
    }
  }
}
